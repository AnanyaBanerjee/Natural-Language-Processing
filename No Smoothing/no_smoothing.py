...
#function to create bigram with no smoothing
def create_bigrams_with_no_smoothing(corpus_bigram):
    #P[w2/w1]= Count(w1, w2)/ Count(w1)
    for w in corpus_bigram:
        w1=w[0]
        w2=w[1]
        #print("word1", w1, " word2", w2)
        prob=counts_bi[str([w1,w2])] / counts_uni[w1]
        bigram_dict[str([w1,w2])]=prob
        ...
        if type_of_smoothing=="no smoothing":
            for x in token_:
                if str(x) in key_b:
                    ...
                    #print("for word", x, "bigram_dict[x]",bigram_dict[str(x)] )
                else:
                    prob=0
                #print("x is",x, "and prob is", prob)
        elif type_of_smoothing=="add one smoothing":
...
create_unigram(vocab, counts, "no smoothing", len(corpus_unigram))
...
create_bigrams_with_no_smoothing(corpus_bigram)
...
#bigram + no smoothing
prob_of_sentence=find_prob_of_sentence(sentence, "bigram", "no smoothing", len(unique_corpus_bigram), len(corpus_bigram))
...
df.to_csv("bigrams_no_smoothing.csv", index=False)
...
df1.to_csv("unigrams_no_smoothing.csv", index=False)
...
df3.to_csv("counts_unigram_no_smoothing.csv", index=False)
...
df2.to_csv("counts_bigram_no_smoothing.csv", index=False)
