...
def perform_coref_resolution(dict_lines_for_each_para):
    ...
    for docs in list(dict_lines_for_each_para.values()):
        #docs=dict_lines_for_each_para['p0']
        ...
        for x in docs:
            ...
            if doc._.has_coref and len(doc._.coref_clusters)>=1:
                coref_cluster.append(doc._.coref_clusters)
                lc.append(x)
                c1=str(doc._.coref_clusters[0][0])
                c2=str(doc._.coref_clusters[0][1])
                
                if c1!=c2:
                    #find c2 and replace it with c1 in x
                    x1=x.split(" ")
                    if c2 in x1:
                        ind=x1.index(c2)
                        x1[ind]=c1
        ...
        CC.append(coref_cluster)
        line_CC.append(lc)
        new_line_CC.append(nlc)
...
def write_to_json(LINES_FINAL):
    ...
    for doc in list(LINES_FINAL.keys()):
        ...
        with open("Output/buy_"+doc+".json", "w") as write_file:
...
def write_json_files_dp(doc_name, rels_tuples):
    ...
    # saving the object b which is json object
    with open("Output/buy_dp_"+doc_name+".json", "w") as write_file:
        json.dump(json_dict, write_file, indent=4)
...
def write_to_json_file(LINES_FINAL_I,PATTERNS_FINAL_I,SEEDS_FINAL_I, doc_name):
    ...
    # saving the object b which is json object
    with open("Output/buy_sf_"+doc_name+".json", "w") as write_file:
        json.dump(json_dict, write_file, indent=4)
...
def use_dp_to_Extract_reln(dict_lines_for_each_para):
    ...
    lis=['bought','buys','buy','acquires','acquire','acquired']
    #reln tuple is [ sub, predicate,o bj]
    reln_tuple=[]
...
def perform_relationship_extraction(data, seeds, file_name,word_embeddings, emb_keys):
    ...
    """    
    new_lines_with_CC=perform_coref_resolution(dict_lines_for_each_para)
    
    i=0
    for key, value in dict_lines_for_each_para.items():
        dict_lines_for_each_para[key] = new_lines_with_CC[i]
        i+=1
    ...
    for seed in seeds:
        ...
        pat, pat_str=calc_confidence_score(seed, ls, pat, pat_str, seeds, dict_lines_for_each_para )
        #refine patterns
...
#df_pat.to_csv("PATTERNS_buy.csv",index=False)
    
df_seed=pd.DataFrame(SEEDS_FINAL, columns=col2)
#df_seed.to_csv("SEEDS_buy.csv",index=False)
...
for doc_name in all_files:
    ...
    with open('Output/buy_dp_'+doc_name+'.json') as f2: 
      f2data = f2.read() 
    
    
    with open('Output/buy_sf_'+doc_name+'.json') as f1: 
      f1data = f1.read() 
     
    f1data += "\n"
    f1data += f2data
    
    with open ('Output/buy_'+doc_name+'.json', 'a') as f3: 
      f3.write(f1data)
    
    if os.path.exists('Output/buy_sf_'+doc_name+'.json'):
      os.remove('Output/buy_sf_'+doc_name+'.json')
    
    if os.path.exists('Output/buy_dp_'+doc_name+'.json'):
      os.remove('Output/buy_dp_'+doc_name+'.json')
