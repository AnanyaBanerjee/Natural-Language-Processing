...
def use_dp_to_Extract_reln(dict_lines_for_each_para):
    lines=list(dict_lines_for_each_para.values())
    lines=[item for sublist in lines for item in sublist]
    if [] in lines:
        lines.remove([])
    ...
    lis=['work', 'works','worked', 'toil','labor','toils','toiled','labors','labored']
    #reln tuple is [ sub, obj, line]
    reln_tuple=[]
    ...
    for x in lines:
        ...
        new_entities=[[h.text, h.label_] for h in doc.ents]
        ne=dict()
        for f in new_entities:
            ne[f[0]]=f[1]
        ...
        for token in doc:
            ...
            if token.text in lis:
                ...
                #if we only get subject then check for pobj's children which will give you the final ans
                """
                if sub!="none" and obj=="none":
                    print("children", children)
                    for c in children:
                        ch=[child for child in c.children]
                        if len(ch)!=0:
                            print("ch is", ch, "x is", x)
                            obj=str(ch[0])
                            r=[sub, token.text, obj]
                            reln_tuple.append(r)
                            break
                """
                if sub!="none" and obj=="none":
                    ...
                    for c in children:
                        ...
                        for ch1 in ch:
                            if ch1.dep_=='pobj':
                                ch2=[child for child in ch1.children]
                                if len(ch2)==0:
                                    obj=str(ch1)
                                    break
                                else:
                                  for z in ch2:
                                    if str(z) in list(ne.keys()):
                                      if ne[str(z)]=='LOC' or ne[str(z)]=='ORG':
                                          obj=str(z)
                                          break
                                r=[sub, obj, x]
                                reln_tuple.append(r)
...
#function to apply stanford open extraction tool to get more patterns
def apply_stanford_openie(dict_lines_for_each_para):
   ...
   with StanfordOpenIE() as client:
        ...
        for text in lines:
            ...
            for triple in client.annotate(text):
                ...
                if triple['relation']=='work':
                    print('\n, Text: %s.' % text)
                    print('\n, |-', triple)
                    seed_tuples.append([triple["subject"], triple["object"]])
                    line_with_seeds.append(text)
                    pat.append('work')
                
                if triple['relation']=='works':
                    print('\n, Text: %s.' % text)
                    print('\n, |-', triple)
                    seed_tuples.append([triple["subject"], triple["object"]])
                    line_with_seeds.append(text)
   ...
   return seed_tuples, line_with_seeds, pat


           
#function to write to a json file
def write_json_files_dp(doc_name, rels_tuples):
    ...
    with open("Output/work_dp_"+doc_name+".json", "w") as write_file:
...
def write_to_json(LINES_FINAL):
    ...
    for doc in list(LINES_FINAL.keys()):
        ...
        # saving the object b which is json object
        with open("Output/work_i_"+doc+".json", "w") as write_file:
            json.dump(json_dict, write_file, indent=4)

  
def write_to_json_file(LINES_FINAL_I,PATTERNS_FINAL_I,SEEDS_FINAL_I, doc_name):
    print("@@@ Writing into json file")
    #writing to a json file
    #Json Dictionary
    json_dict=dict()
    ...
    #LINES_FINAL_I,PATTERNS_FINAL_I,SEEDS_FINAL_I
    
    # {key:value mapping} 
    json_dict["document"]=doc_name
    #extraction list
    ext=[]
    for i in range(len(LINES_FINAL_I)):
        dict_=dict()
        dict_["template"]="WORK"
        dict_["sentences"]=LINES_FINAL_I[i]
        arg=dict()
        arguments=list(range(2))
        s=SEEDS_FINAL_I[i]
        #arguments=len(m.group(0))
    ...
    # saving the object b which is json object
    with open("Output/work_sf_"+doc_name+".json", "w") as write_file:
        json.dump(json_dict, write_file, indent=4)
...
def perform_relationship_extraction(data, seeds, file_name,word_embeddings, emb_keys):
    ...
    reln_extracted_tuples=use_dp_to_Extract_reln(dict_lines_for_each_para)
    
    #write dep parsing extracted rel to json file
    write_json_files_dp(file_name, reln_extracted_tuples)
    ...
    S_Tup, Line_W_S, Pat_T=apply_stanford_openie(dict_lines_for_each_para)
    
    return PATTERNS, SEEDS, file_lines, S_Tup, Line_W_S, Pat_T
...
for file_name in list(files_data.keys()):
    ...
    seeds=[ ("Abraham Thomas Lincoln"," U.S."),
           ...
           ("Musk","Mars Oasis"),
           ("Griffin","CIA"),
           ("Hitler","Greater Germany"),
           ("Watson","CTR"),
           ("Thomas Watson","IBM World Trade Corporation"),
           ("Mahatma Gandhi","India"),
           ("Ram Nath Kovind","India"),
           ("David Rockefeller","Chase Manhattan"),
           ("J.P.Morgran","Morgan Stanley "),
           ("Lawrence's","Virginia Militia"),
           ("General Edward Braddock","British expedition"),
           ("Thomas Gage","British Forces"),
           ("Peter Stuyvesant","New Netherland"),
           ("Anthony Colv","Dutch"),
           ("Oprah Winfrey","he Oprah Winfrey Show"),
           ("Martin Eberhard","Tesla"),
           ("Tesla Motors","Martin Eberhard and Marc Tarpenning"),
           ("eBay","Jeff Skoll"),
           ("J. C. Penney","Sam Walton"),
           ("H. Lee Scott"," Walmart"),
           ("Bryce Jordan","university"),
           ("Mount Rutford","UT Dallas"),
           ("Buffett","Berkshire Hathaway"),
           ("Buffett","Buffett-Falk & Co"),
           ("George Washington","United States"),
           ("Mohandas Karamchand Gandhi","Indian independence movement"),
           ("Gandhi","Indian National Congress"),
           ("Karamchand Uttamchand Gandhi","Porbandar"),
    ...
    PT, Final_Seeds, File_lines, S_Tup, Line_W_S, Pat_T=perform_relationship_extraction(data, seeds, file_name,word_embeddings,emb_keys) 
    PATTERNS_FINAL.append(PT)
    SEEDS_FINAL.append(Final_Seeds)
    print ("\n, Pat",PT)
    print ("\n, See",Final_Seeds)
    LINES_FINAL[file_name]=File_lines 
    PATTERNS_FINAL_I.append(Pat_T)
    SEEDS_FINAL_I.append(S_Tup)
    LINES_FINAL_I.append(Line_W_S)
    
    write_to_json_file(Line_W_S, Pat_T, S_Tup, file_name )
...
#df_pat.to_csv("PATTERNS_work.csv",index=False)
    
df_seed=pd.DataFrame(SEEDS_FINAL, columns=col2)
#df_seed.to_csv("SEEDS_work.csv",index=False)
...
for doc_name in all_files:
    ...
    with open('Output/work_sf_'+doc_name+'.json') as f2: 
      f2data = f2.read() 
    
    with open('Output/work_dp_'+doc_name+'.json') as f1: 
      f1data = f1.read() 
     
    f1data += "\n"
    f1data += f2data
    
    with open ('Output/work_'+doc_name+'.json', 'a') as f3: 
      f3.write(f1data)
    
    if os.path.exists('Output/work_sf_'+doc_name+'.json'):
      os.remove('Output/work_sf_'+doc_name+'.json')
    
    if os.path.exists('Output/work_dp_'+doc_name+'.json'):
      os.remove('Output/work_dp_'+doc_name+'.json')
    
    if os.path.exists('Output/work_i_'+doc_name+'.json'):
      os.remove('Output/work_i_'+doc_name+'.json')
